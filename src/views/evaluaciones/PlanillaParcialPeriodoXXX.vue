            ...
            <div v-if="idPlanilla != null">
              ...
              <b-row v-else-if="planillita == 5">
                <b-col>
                  ...
                  <vue-good-table :columns="encabColumnas3" :rows="notasPlanillaCompor" :lineNumbers="true" ref="table">
                    <template #table-row="{ column, row, rowIndex }">
                      <template v-if="column.field === 'definitivacompor'">
                        <input v-model="row.definitivacompor" @keydown="handleKeyNavigation($event, rowIndex, row.originalIndex)" :ref="'input-' + row.originalIndex" />
                      </template>
                      <template v-else>
                        {{ row[column.field]}}
                      </template>
...
<script>
  ...
  export default {
    ...
    methods: {
      handleKeyNavigation(event, rowIndex, i) {
        if (event.key === "ArrowDown") {
          const nextInput = this.$refs[`input-${i + 1}`]
          if (nextInput) nextInput.focus()
        } else if (event.key === "ArrowUp") {
          const previousInput = this.$refs[`input-${i - 1}`]
